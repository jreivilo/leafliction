...
def apply_augmentations(file_path: str, save_to_augmented_dir: bool = False) -> None:
    ...
    # Save each augmented image in both the original folder and `augmented_directory` if requested
    for suffix, img in augmentations.items():
        ...
        if save_to_augmented_dir:
            # Only create `augmented_directory` if `save_to_augmented_dir` is True
